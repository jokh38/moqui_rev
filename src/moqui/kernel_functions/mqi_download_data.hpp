...
template <typename R>
CUDA_GLOBAL void download_scorers(mqi::node_t<R> *g_node_mother,
                                  uint16_t g_node_id, uint16_t scorer_id,
                                  R *deposit);
template <typename R>
...
void download_node(mqi::node_t<R> *c_node, mqi::node_t<R> *&g_node) {
  ...
  gpu_err_chk(
      cudaMemcpy(&tmp, g_node, sizeof(mqi::node_t<R>), cudaMemcpyDeviceToHost));

  // printf("node:%p, n_scorers: %d, n_children: %d, *p children:%p, *p
  // scorers:%p\n",
  //        g_node, tmp.n_scorers, tmp.n_children, tmp.children,
  //        tmp.scorers_data);
  ...
  if (tmp.n_scorers > 0) {
    ...
    gpu_err_chk(cudaMemcpy(scrs, tmp.scorers_data,
    ...
    for (int i = 0; i < tmp.n_scorers; ++i) {
      ...
      gpu_err_chk(
          cudaMemcpy(c_node->scorers[i]->data_, scrs[i],
  ...
  if (tmp.n_children > 0) {
    ...
    gpu_err_chk(cudaMemcpy(children, tmp.children,
