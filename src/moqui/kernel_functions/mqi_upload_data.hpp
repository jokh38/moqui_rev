...
template <typename R>
void upload_materials(mqi::material_t<R> *c_materials,
                      mqi::material_t<R> *&g_materials, uint16_t n_materials);
template <typename R>
void upload_vertices(mqi::vertex_t<R> *src, mqi::vertex_t<R> *&dest, size_t h0,
                     size_t h1);
...
template <typename R>
void upload_scorer_idx(int32_t *c_scorer_idx, int32_t *&g_scorer_idx,
                       uint32_t n_scorer_idx) {
  gpu_err_chk(cudaMalloc(&g_scorer_idx, n_scorer_idx * sizeof(int32_t)));
  gpu_err_chk(cudaMemcpy(g_scorer_idx, c_scorer_idx,
                         n_scorer_idx * sizeof(int32_t),
                         cudaMemcpyHostToDevice));
}
template <typename R>
CUDA_GLOBAL void print_vertex(mqi::beamsource<R> *beamsource) {
  for (int i = 0; i < beamsource->beamlet_size_; i++) {
    printf("cdf %d %lu %lu\n", i, beamsource->array_cdf_[i],
           beamsource->array_beamlet_[i]);
  }
  printf("beamsizes %d\n", beamsource->beamlet_size_);
}

template <typename R>
CUDA_GLOBAL void add_beamlet(mqi::beamsource<R> *beamsource, size_t *cdf,
                             size_t *beamlet, size_t beamlet_size) {
...
void upload_beamsource(mqi::beamsource<R> src, mqi::beamsource<R> *&dest) {
  ...
  gpu_err_chk(cudaMemcpy(dest, &src, sizeof(mqi::beamsource<R>),
                         cudaMemcpyHostToDevice));
  size_t *d_cdf;
  size_t *d_beamlet;
  gpu_err_chk(cudaMalloc(&d_cdf, src.beamlet_size_ * sizeof(size_t)));
  gpu_err_chk(cudaMemcpy(d_cdf, src.array_cdf_,
                         src.beamlet_size_ * sizeof(size_t),
                         cudaMemcpyHostToDevice));
  gpu_err_chk(cudaMalloc(&d_beamlet, src.beamlet_size_ * sizeof(size_t)));
  gpu_err_chk(cudaMemcpy(d_beamlet, src.array_beamlet_,
                         src.beamlet_size_ * sizeof(size_t),
                         cudaMemcpyHostToDevice));
  ...
  //        src.array_beamlet_[i]);
  //    }
}
template <typename R>
CUDA_GLOBAL void add_node_geometry(
    mqi::node_t<R> *node, R *xe, mqi::ijk_t nxe, R *ye, mqi::ijk_t nye, R *ze,
    mqi::ijk_t nze, mqi::density_t *data, mqi::mat3x3<R> *rotation_matrix_inv,
    mqi::mat3x3<R> *rotation_matrix_fwd, mqi::vec3<R> *translation_vector,
    uint16_t n_children = 0, mqi::node_t<R> **children = nullptr) {

  // std::cout << "Adding geometry node .. : Node --> " << node << ", number of
...
CUDA_GLOBAL void add_node_scorers(
    mqi::node_t<R> *node, uint16_t n_scorers = 0,
    ...
    mqi::scorer_t *scorer_types = nullptr, uint32_t *scorer_sizes = nullptr,
    std::string *scorer_names = nullptr, mqi::fp_compute_hit<R> *fp = nullptr,
  ...
  // std::cout << "Adding scorers node .. : Node --> " << node << ", number of
  // children --> " << n_scorers << std::endl;
  ...
  for (int i = 0; i < n_scorers; i++) {
    ...
    node->scorers[i]->roi_ =
        new mqi::roi_t(roi_method[i], roi_original_length[i], roi_length[i],
                       roi_start[i], roi_stride[i], roi_acc_stride[i]);
...
void upload_node(mqi::node_t<R> *c_node, mqi::node_t<R> *&g_node) {
  ...
  // std::cout << "Uploading data to node.. : Allocated node --> " << g_node <<
  // std::endl;
  ...
  gpu_err_chk(cudaMemcpy(x_edges, c_node->geo->get_x_edges(),
                         (dim.x + 1) * sizeof(R), cudaMemcpyHostToDevice));
  gpu_err_chk(cudaMemcpy(y_edges, c_node->geo->get_y_edges(),
                         (dim.y + 1) * sizeof(R), cudaMemcpyHostToDevice));
  gpu_err_chk(cudaMemcpy(z_edges, c_node->geo->get_z_edges(),
                         (dim.z + 1) * sizeof(R), cudaMemcpyHostToDevice));
  ...
  if (c_node->n_scorers > 0) {
    ...
    gpu_err_chk(cudaMalloc(&d_scorers_types,
                           c_node->n_scorers * sizeof(mqi::scorer_t)));
    gpu_err_chk(
        cudaMalloc(&d_scorers_size, c_node->n_scorers * sizeof(uint32_t)));
    gpu_err_chk(cudaMalloc(&d_scorers_data,
                           c_node->n_scorers * sizeof(mqi::key_value *)));
    gpu_err_chk(
        cudaMalloc(&d_fp, c_node->n_scorers * sizeof(mqi::fp_compute_hit<R>)));
    gpu_err_chk(
        cudaMalloc(&d_roi_start, c_node->n_scorers * sizeof(uint32_t *)));
    gpu_err_chk(
        cudaMalloc(&d_roi_stride, c_node->n_scorers * sizeof(uint32_t *)));
    gpu_err_chk(
        cudaMalloc(&d_roi_acc_stride, c_node->n_scorers * sizeof(uint32_t *)));
    gpu_err_chk(
        cudaMalloc(&d_roi_length, c_node->n_scorers * sizeof(uint32_t)));
    gpu_err_chk(cudaMalloc(&d_roi_original_length,
                           c_node->n_scorers * sizeof(uint32_t)));
    gpu_err_chk(cudaMalloc(&d_roi_method,
    ...
    if (c_node->scorers[0]->score_variance_) {
      ...
      gpu_err_chk(cudaMalloc(&d_scorers_count,
                             c_node->n_scorers * sizeof(mqi::key_value *)));
      gpu_err_chk(cudaMalloc(&d_scorers_mean,
                             c_node->n_scorers * sizeof(mqi::key_value *)));
      gpu_err_chk(cudaMalloc(&d_scorers_variance,
                             c_node->n_scorers * sizeof(mqi::key_value *)));
    }
    ...
    for (int i = 0; i < c_node->n_scorers; i++) {
      ...
      gpu_err_chk(
          cudaMalloc(&h_scorers_data[i], c_node->scorers[i]->max_capacity_ *
      ...
      //            c_node->scorers[i]->max_capacity_);
      gpu_err_chk(
          cudaMemcpy(h_scorers_data[i], c_node->scorers[i]->data_,
                     c_node->scorers[i]->max_capacity_ * sizeof(mqi::key_value),
                     cudaMemcpyHostToDevice));
      gpu_err_chk(
          cudaMalloc(&h_roi_start[i],
                     c_node->scorers[i]->roi_->length_ * sizeof(uint32_t)));
      gpu_err_chk(
          cudaMalloc(&h_roi_stride[i],
                     c_node->scorers[i]->roi_->length_ * sizeof(uint32_t)));
      gpu_err_chk(
          cudaMalloc(&h_roi_acc_stride[i],
                     c_node->scorers[i]->roi_->length_ * sizeof(uint32_t)));
      if (c_node->scorers[i]->roi_->length_ > 0) {
        gpu_err_chk(
            cudaMemcpy(h_roi_start[i], c_node->scorers[i]->roi_->start_,
                       c_node->scorers[i]->roi_->length_ * sizeof(uint32_t),
                       cudaMemcpyHostToDevice));
        gpu_err_chk(
            cudaMemcpy(h_roi_stride[i], c_node->scorers[i]->roi_->stride_,
                       c_node->scorers[i]->roi_->length_ * sizeof(uint32_t),
                       cudaMemcpyHostToDevice));
        gpu_err_chk(cudaMemcpy(
            h_roi_acc_stride[i], c_node->scorers[i]->roi_->acc_stride_,
            c_node->scorers[i]->roi_->length_ * sizeof(uint32_t),
            cudaMemcpyHostToDevice));
      } else {
      ...
      if (c_node->scorers[i]->score_variance_) {
        gpu_err_chk(
            cudaMalloc(&h_scorers_count[i], c_node->scorers[i]->max_capacity_ *
                                                sizeof(mqi::key_value)));
        ...
        gpu_err_chk(
            cudaMalloc(&h_scorers_mean[i], c_node->scorers[i]->max_capacity_ *
                                               sizeof(mqi::key_value)));
        ...
        gpu_err_chk(
            cudaMalloc(&h_scorers_var[i], c_node->scorers[i]->max_capacity_ *
                                              sizeof(mqi::key_value)));
    ...
    gpu_err_chk(cudaMemcpy(d_roi_start, h_roi_start,
                           c_node->n_scorers * sizeof(uint32_t *),
                           cudaMemcpyHostToDevice));
    gpu_err_chk(cudaMemcpy(d_roi_stride, h_roi_stride,
                           c_node->n_scorers * sizeof(uint32_t *),
                           cudaMemcpyHostToDevice));
    ...
    gpu_err_chk(cudaMemcpy(d_scorers_types, scorers_types,
                           c_node->n_scorers * sizeof(mqi::scorer_t),
                           cudaMemcpyHostToDevice));
    gpu_err_chk(cudaMemcpy(d_scorers_size, scorers_size,
                           c_node->n_scorers * sizeof(uint32_t),
                           cudaMemcpyHostToDevice));
    gpu_err_chk(cudaMemcpy(d_fp, fp,
                           c_node->n_scorers * sizeof(mqi::fp_compute_hit<R>),
                           cudaMemcpyHostToDevice));
    gpu_err_chk(cudaMemcpy(d_roi_length, roi_length,
                           c_node->n_scorers * sizeof(uint32_t),
                           cudaMemcpyHostToDevice));
  ...
  if (c_node->n_children >= 1) {
    ...
    gpu_err_chk(
        cudaMalloc(&d_children, c_node->n_children * sizeof(mqi::node_t<R> *)));

    for (int i = 0; i < c_node->n_children; ++i) {
      gpu_err_chk(cudaMalloc(&h_children[i], sizeof(mqi::node_t<R>)));
      printf("Uploading data to node.. : g_node --> %p, d_children --> %p, "
             "h_children[%d] --> %p\n",
             g_node, d_children, i, h_children[i]);
      // std::cout << "Uploading data to node.. : Allocated d_children --> " <<
      // d_children << std::endl; std::cout << "Uploading data to node.. :
      // Allocated h_children " << "[" + std::to_string(i) << "] --> " <<
      // h_children[i] << std::endl;
    }
    gpu_err_chk(cudaMemcpy(d_children, h_children,
  ...
  mc::add_node_geometry<R>
      <<<1, 1>>>(g_node, x_edges, dim.x + 1, y_edges, dim.y + 1, z_edges,
  ...
  if (c_node->n_scorers > 0) {
    mc::add_node_scorers<R><<<1, 1>>>(
        g_node, c_node->n_scorers, d_scorers_data, d_scorers_count,
        d_scorers_mean, d_scorers_variance, d_scorers_types, d_scorers_size,
        d_scorers_name, d_fp, d_roi_method, d_roi_original_length, d_roi_length,
        d_roi_start, d_roi_stride, d_roi_acc_stride);
  } else {
  ...
  gpu_err_chk(
      cudaFree(d_scorers_types)); // it's working, but not sure it is required
  gpu_err_chk(
      cudaFree(d_scorers_size)); // it's working, but not sure it is required
  ...
  //    gpu_err_chk(cudaFree(d_roi));             // it's working, but not sure
  //    it is required
} // upload_node
...
void upload_materials(mqi::material_t<R> *c_materials,
  ...
  gpu_err_chk(
      cudaMalloc(&g_materials, n_materials * sizeof(mqi::material_t<R>)));
  gpu_err_chk(cudaMemcpy(g_materials, c_materials,
                         n_materials * sizeof(mqi::material_t<R>),
                         cudaMemcpyHostToDevice));
}
...
void upload_vertices(mqi::vertex_t<R> *src, mqi::vertex_t<R> *&dest, size_t h0,
  ...
  gpu_err_chk(cudaMemcpy(dest, src, (h1 - h0) * sizeof(mqi::vertex_t<R>),
                         cudaMemcpyHostToDevice));
}
...
void upload_ct(int16_t *c_ct, int16_t *&g_ct, uint32_t size_) {
  ...
  gpu_err_chk(
      cudaMemcpy(g_ct, c_ct, size_ * sizeof(int16_t), cudaMemcpyHostToDevice));
}

void upload_density(float *c_density, float *&g_density, uint32_t size_) {
  gpu_err_chk(cudaMalloc(&g_density, size_ * sizeof(float)));
  gpu_err_chk(cudaMemcpy(g_density, c_density, size_ * sizeof(float),
                         cudaMemcpyHostToDevice));
}

void upload_density(__half *c_density, __half *&g_density, uint32_t size_) {
  gpu_err_chk(cudaMalloc(&g_density, size_ * sizeof(__half)));
  gpu_err_chk(cudaMemcpy(g_density, c_density, size_ * sizeof(__half),
                         cudaMemcpyHostToDevice));
}
...
void upload_scorer_offset_vector(uint16_t *src, uint16_t *&dest, size_t size_) {
  ...
  gpu_err_chk(
      cudaMemcpy(dest, src, size_ * sizeof(uint16_t), cudaMemcpyHostToDevice));
}
...
void upload_scorer_offset_vector(uint32_t *src, uint32_t *&dest, size_t size_) {
  ...
  gpu_err_chk(
      cudaMemcpy(dest, src, size_ * sizeof(uint32_t), cudaMemcpyHostToDevice));
}

void upload_scorer_offset_vector(unsigned long *src, unsigned long *&dest,
                                 size_t size_) {
  gpu_err_chk(cudaMalloc(&dest, size_ * sizeof(uint32_t)));
  gpu_err_chk(
      cudaMemcpy(dest, src, size_ * sizeof(uint32_t), cudaMemcpyHostToDevice));
...
void upload_scorer_offset_vector(unsigned long long int *src,
                                 unsigned long long int *&dest, size_t size_) {
  gpu_err_chk(cudaMalloc(&dest, size_ * sizeof(uint32_t)));
  gpu_err_chk(
      cudaMemcpy(dest, src, size_ * sizeof(uint32_t), cudaMemcpyHostToDevice));
}
