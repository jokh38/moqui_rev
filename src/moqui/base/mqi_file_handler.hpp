...
public:
  ...
  void read_mask_files() {
    ...
    if (mask_filenames.empty()) {
      throw std::runtime_error(
          "Mask filelist are required for masking scorers.");
    }
  ...
  mqi::roi_t *mask_to_roi() {
    ...
    for (ijk_t z_ind = 0; z_ind < this->ct_dim.z; z_ind++) {
      for (ijk_t y_ind = 0; y_ind < this->ct_dim.y; y_ind++) {
        for (ijk_t x_ind = 0; x_ind < this->ct_dim.x; x_ind++) {
          if (this->mask_total[ijk2cnb(x_ind, y_ind, z_ind)] == 1 &&
              !ind_started) {
            ind_started = true;
            start_vec.push_back(ijk2cnb(x_ind, y_ind, z_ind));
          }
          if (this->mask_total[ijk2cnb(x_ind, y_ind, z_ind)] == 0 &&
              ind_started) {
            ind_started = false;
    ...
    mqi::roi_t *roi = new mqi::roi_t(method, original_size, length, start,
                                     stride, acc_stride);
    return roi;
  }
};
...
public:
    ...
    for (int line = 0; line < parameters_total.size(); line++) {
      ...
      if (pos != std::string::npos) {
        ...
        if (strcasecmp(option.c_str(), option_in.c_str()) == 0) {
          value = trim_copy(
              parameters_total[line].substr(pos + 1, std::string::npos));
          return value;
        }
      }
    }
  ...
  CUDA_HOST
  std::vector<std::string> get_string_vector(std::string option,
  ...
  float get_float(std::string option, float default_value) {
    float value =
        std::atof(get_string(option, std::to_string(default_value)).c_str());
    return value;
  }

  CUDA_HOST
  std::vector<float> get_float_vector(std::string option,
                                      std::string delimter) {
  ...
  int get_int(std::string option, int default_value) {
    int value =
        std::atoi(get_string(option, std::to_string(default_value)).c_str());
    return value;
  }
  ...
  bool get_bool(std::string option, bool default_value) {
    ...
    bool value =
        strcasecmp(temp.c_str(), "true") == 0 || std::atoi(temp.c_str()) != 0;
    return value;
  }
