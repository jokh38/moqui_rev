#ifndef MQI_UTILS_HPP
#define MQI_UTILS_HPP
...
inline std::string
trim_right_copy(const std::string &s,
...
inline std::string
trim_left_copy(const std::string &s,
               const std::string &delimiters = " \f\n\r\t\v\0\\") {
...
inline T interp_linear(const std::map<T, std::array<T, S>> &db, const T x,
...
/// \param y_col column index of map for y-values to be used for interpolation
template <typename T, size_t S>
inline T interp_linear(const std::vector<std::array<T, S>> &db, const T x,
                       const size_t x_col = 0, const size_t y_col = 1) {
  if (x <= db[0][x_col])
    return db[0][y_col];
...
/// \note from gpmc code
inline float TableInterpolation(float *const vector_X, float *const vector_Y,
                                const float x, const int npoints,
                                int order = 4) {
  float result;
  ...
  for (int is = j; is < j + order; is++) {
    ...
    for (int il = j; il < j + order; il++) {
      if (il != is)
        lambda[is] =
            lambda[is] * (x - vector_X[il]) / (vector_X[is] - vector_X[il]);
    }
...
CUDA_DEVICE
vec2<uint32_t> start_and_length(const uint32_t &n_threads,
  ...
  ret.x =
      quotient * thread_id + ((thread_id >= remainder) ? remainder : thread_id);
...
CUDA_HOST_DEVICE
int32_t lower_bound_cpp(const int32_t *arr, const int32_t &len,
                        const int32_t &value) {
  int32_t first = 0;
  int32_t count = len;
  int32_t step;
