...
#include "mqi_p_ionization.hpp"
#include "mqi_physics_list.hpp"
#include "mqi_po_elastic.hpp"
#include "mqi_po_inelastic.hpp"
#include "mqi_pp_elastic.hpp"

namespace mqi {

template <typename R> class fippel_physics : public physics_list<R> {
public:
  ...
  CUDA_HOST_DEVICE
  fippel_physics()
      : p_ion(0.1, 299.6, 0.5, mqi::cs_p_ion_table,
              mqi::restricted_stopping_power_table, mqi::range_steps),
  ...
  ///< step length
  CUDA_HOST_DEVICE
  virtual void stepping(track_t<R> &trk, track_stack_t<R> &stk, mqi_rng *rng,
                        const R &rho_mass, material_t<R> &mat,
    ...
    current_min_step = current_min_step * mat.stopping_power_ratio(rel.Ek) *
                       mat.rho_mass / this->units.water_density;
    // current_min_step   = current_min_step * 1 * mat.rho_mass /
    // this->units.water_density;
    current_min_step =
        (current_min_step <= max_loss_step) ? current_min_step : max_loss_step;
    ...
    mqi::relativistic_quantities<R> rel_de(trk.vtx0.ke - max_loss_energy,
                                           units.Mp);
    R cs2[4] = {
        p_ion.cross_section(rel_de, mat), pp_e.cross_section(rel_de, mat),
    ...
    // R step_limit = current_min_step * this->units.water_density / (1 *
    // mat.rho_mass);
    ...
    printf("\tcs1: %.3f vs cs2: %.3f, prob: %.3f, mfp: %.3f\n", cs1_sum,
           cs2_sum, prob, mfp);
    printf("\t%.3f MeV: current_min_step: %.3f, max_loss_step: %.3f, "
           "distance_to_boundary: %.3f\n",
    ...
    if (distance_to_boundary < mfp && distance_to_boundary < step_limit) {
      ...
      assert_track<R>(trk, 0);
                ...
                mqi::mqi_abs(mfp - distance_to_boundary) <
                    mqi::geometry_tolerance) &&
               (mfp < step_limit ||
                mqi::mqi_abs(mfp - step_limit) < mqi::geometry_tolerance)) {
      int p = 0;
