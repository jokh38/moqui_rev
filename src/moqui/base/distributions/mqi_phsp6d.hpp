#ifndef MQI_PHSP6D_HPP
#define MQI_PHSP6D_HPP
...
template <typename T> class phsp_6d : public pdf_Md<T, 6> {
...
public:
  ...
  /// Constructor to initializes mean, sigma, rho, and random engine
  CUDA_HOST_DEVICE
  phsp_6d(const std::array<T, 6> &m, const std::array<T, 6> &s,
  ...
  virtual std::array<T, 6> operator()(std::default_random_engine *rng) {
    ...
    phsp[3] += pdf_Md<T, 6>::sigma_[3] *
               (rho_[0] * Ux + Vx * std::sqrt(1.0 - rho_[0] * rho_[0]));
    phsp[4] += pdf_Md<T, 6>::sigma_[4] *
