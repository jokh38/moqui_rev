#ifndef MQI_PHSP6D_RAY_HPP
#define MQI_PHSP6D_RAY_HPP
...
template <typename T> class phsp_6d_ray : public pdf_Md<T, 6> {
...
public:
  ...
  /// \param s[3,4,5]: std  spot-direction of x,y,z. s[5] is ignored but
  /// calculated internally \param r[0] : x, xp correlation \param r[1] : y, yp
  ...
  /// Constructor to initializes mean, sigma, rho, and random engine
  CUDA_HOST_DEVICE
  phsp_6d_ray(const std::array<T, 6> &m, const std::array<T, 6> &s,
              const std::array<T, 6> &r, const float source_position)
      : pdf_Md<T, 6>(m, s), rho_(r) {
    // #if !defined(__CUDACC__)
    //         gen_.seed(std::chrono::system_clock::now().time_since_epoch().count());
    //         gen_.seed(1000);
  ...
  virtual std::array<T, 6> operator()(std::default_random_engine *rng) {
    ...
    T norm =
        std::sqrt(phsp[3] * phsp[3] + phsp[4] * phsp[4] + phsp[5] * phsp[5]);
    //        printf(
    //          "dir.x %f dir.y %f dir.z %f norm %f th20 %f\n", phsp[3],
    //          phsp[4], phsp[5], norm, th20);
    phsp[3] /= norm;
    phsp[4] /= norm;
    phsp[5] /= norm;
    ...
    phsp[3] =
        phsp[3] * std::cos(Vx * std::sqrt(th20_x / 2)) +
        std::sqrt(1 - phsp[3] * phsp[3]) * std::sin(Vx * std::sqrt(th20_x / 2));
    phsp[4] =
        phsp[4] * std::cos(Vy * std::sqrt(th20_y / 2)) +
  ...
  //        phsp[2] += pdf_Md<T, 6>::sigma_[2] * Uz;
  //
  //        phsp[3] += (pdf_Md<T, 6>::sigma_[3] * Vx + pdf_Md<T, 6>::sigma_[0] *
  //        Ux * rho_[0]); phsp[4] += (pdf_Md<T, 6>::sigma_[4] * VY + pdf_Md<T,
  //        6>::sigma_[1] * Uy * rho_[1]);
  //        //        phsp[3] +=
  //        //          pdf_Md<T, 6>::sigma_[3] * (rho_[0] * Ux + Vx *
  //        std::sqrt(1.0 - rho_[0] * rho_[0]));
  //        //        phsp[4] +=
  //        //          pdf_Md<T, 6>::sigma_[4] * (rho_[1] * Uy + Vy *
  //        std::sqrt(1.0 - rho_[1] * rho_[1])); phsp[5] = -1.0 * std::sqrt(1.0
  //        - phsp[3] * phsp[3] - phsp[4] * phsp[4]); return phsp;
  //    };
};
