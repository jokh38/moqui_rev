...
#ifndef MQI_PHSP_6D_FANBEAM_HPP
#define MQI_PHSP_6D_FANBEAM_HPP
...
//  phase-space variables are position (x,y,z) and direction (x',y',z') between
//  x_max, x_min and y_max and y_min, etc.
/// \tparam T type of return value
template <typename T> class phsp_6d_fanbeam : public pdf_Md<T, 6> {
...
public:
  ...
  phsp_6d_fanbeam(
      ...
      const std::array<T, 6>
          &s, ///< [sig_x, sig_y, sig_z, sig_x', sig_y', sig_z']
  ...
  /// Constructor to initialize distribution parameters
  /// phase-space variables are sampled same in phsp_6d but they are spread
  /// along x and y.
  CUDA_HOST_DEVICE
  virtual std::array<T, 6> operator()(std::default_random_engine *rng) {
    ...
    phsp[3] =
        dir.x + pdf_Md<T, 6>::sigma_[3] *
                    (rho_[0] * Ux + Vx * std::sqrt(1.0 - rho_[0] * rho_[0]));
    phsp[4] =
        dir.y + pdf_Md<T, 6>::sigma_[4] *
