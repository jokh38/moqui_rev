#ifndef MQI_TREATMENT_MACHINE_HPP
#define MQI_TREATMENT_MACHINE_HPP

/// \file
///
/// Abstraction for treatment machine

#include "mqi_beam_module.hpp"
#include "mqi_beam_module_ion.hpp"
#include "mqi_beamlet.hpp"
#include "mqi_beamline.hpp"
#include "mqi_beamsource.hpp"
...
/// Typically it consists of geometries (beam limiting devices) and sources
/// (particle fluence) This class has three user-implemented methods,
/// create_beamline, create_beamsource, and create_coordinate_transform \tparam
/// T type of phase-space variables, e.g., float or double \note treatment
/// machine is distinguished in its name
template <typename T> class treatment_machine {
protected:
...
public:
  ...
  /// \param ds : pointer of dataset
  /// \param m  : modality type such as RT-Ion Plan, RT-Ion Beam Treatment
  /// Record \param pcoord: coordinate system of beam geometry including gantry,
  /// patient support, etc \param particles_per_history : total number of
  /// histories can be calculated using this number
  ...
  /// \param pcoord: coordinate system of beam geometry including gantry,
  /// patient support, etc \param particles_per_history : total number of
  /// histories can be calculated using this number
  virtual mqi::beamsource<T>
  ...
  /// \return pcoord: coordinate system of beam geometry including gantry,
  /// patient support, etc
  virtual mqi::coordinate_transform<T>
  create_coordinate_transform(const mqi::dataset *ds,
                              const mqi::modality_type m) = 0;
...
protected:
  ...
  /// Returns beamlet
  /// \param s : spot contains energy, x-, and y-position, fwhm, and NP
  /// \param source_to_isocenter_mm : distance between source and isocenter in
  /// mm scale \return beamlet: beamlet created from given parameters
  virtual mqi::beamlet<T>
