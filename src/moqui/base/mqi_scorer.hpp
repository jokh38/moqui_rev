...
template <typename R>
using fp_compute_hit = double (*)(const track_t<R> &, const mqi::cnb_t &,
...
public:
  ...
  ///< Construct with size
  CUDA_HOST_DEVICE
  scorer(const char *name, const uint32_t max_capacity,
         const fp_compute_hit<R> func_pointer)
      : name_(name), max_capacity_(max_capacity),
        current_capacity_(max_capacity), compute_hit_(func_pointer) {
    this->delete_data_if_used();
  }

  CUDA_HOST_DEVICE
  ~scorer() { this->delete_data_if_used(); }
  ...
  void clear_data() {
    ...
    if (this->score_variance_) {
      ...
      std::memset(variance_, 0xff,
                  sizeof(mqi::key_value) * this->max_capacity_);
    }
  }
};
