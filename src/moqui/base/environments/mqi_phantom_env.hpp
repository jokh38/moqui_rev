...
template <typename R> class phantom_env : public x_environment<R> {
public:
...
public:
  ...
  phantom_env(mqi::cli &cli) : x_environment<R>() {
    ...
    if (cli["--score_variance"].size() >= 1) {
      ...
      this->score_variance =
          strcasecmp(v.c_str(), "true") == 0 || std::atoi(v.c_str()) != 0;
    } else {
    ...
    if (cli["--random_seed"].size() >= 1) {
      ...
      this->random_seed = static_cast<int>(
  ...
  virtual void setup_beamsource() {
    mqi::coordinate_transform<R> p_coord(spot_angles,
                                         {0, 0, 0}); // angles, isocenter
    /// Source direction is -Z
    mqi::vec3<R> dir(0, 0, -1);
    ///< Beamlet phse-space distribution
    std::array<R, 6> beamlet_mean = {spot_position[0],
                                     spot_position[1],
                                     spot_position[2],
                                     dir.x,
                                     dir.y,
                                     dir.z};
    std::array<R, 6> beamlet_sigm = {spot_size[0], spot_size[1], 0.0,
                                     0.0,          0.0,          0.0};
    ...
    this->beamsource.append_beamlet(mqi::beamlet<R>(energy, phsp), n_histories,
                                    p_coord);
  ...
  virtual void setup_world() {
...
#endif
    phantom->scorers[0] =
        new mqi::scorer<R>("water_dE_total", nxyz.x * nxyz.y * nxyz.z, fp0);
    ...
    phantom->scorers[0]->roi_ =
        new mqi::roi_t(mqi::DIRECT, nxyz.x * nxyz.y * nxyz.z);

    if (this->score_variance) {
      mqi::key_value *count =
          new mqi::key_value[phantom->scorers[0]->max_capacity_];
      mqi::key_value *vox_mean =
          new mqi::key_value[phantom->scorers[0]->max_capacity_];
      mqi::key_value *vox_variance =
          new mqi::key_value[phantom->scorers[0]->max_capacity_];
  ...
  virtual void run() {
    ...
    if (err != cudaSuccess) {
      std::cout << "CUDA error (upload_vertices): " << cudaGetErrorString(err)
                << std::endl;
      exit(-1);
    }
    ...
    free = 0;
    total = 0;
    mc::upload_scorer_offset_vector(scorer_offset_vector,
                                    d_scorer_offset_vector, h1 - h0);
    cudaMemGetInfo(&free, &total);
    ...
    if (err != cudaSuccess) {
      std::cout << "CUDA error (upload scoerer offset vector): "
                << cudaGetErrorString(err) << std::endl;
      exit(-1);
    }
    gpu_err_chk(cudaMalloc(&worker_threads,
                           threads[1] * threads[0] * sizeof(mqi::thrd_t)));

    free = 0;
    total = 0;
    initialize_threads<<<threads[1], threads[0]>>>(worker_threads,
                                                   threads[1] * threads[0], 0);
    cudaMemGetInfo(&free, &total);
    printf("After thread init total memory %lu MB Free memory %lu MB\n",
           total / (1024 * 1024), free / (1024 * 1024));
    //        initialize_threads<<<threads[1], threads[0]>>>(worker_threads, 0,
    //        0);
    err = cudaGetLastError();
    if (err != cudaSuccess) {
      std::cout << "CUDA error (initialize threads): "
                << cudaGetErrorString(err) << std::endl;
      exit(-1);
    }
    ...
    gpu_err_chk(cudaMemcpy(d_tracked_particles, tracked_particles,
                           sizeof(tracked_particles), cudaMemcpyHostToDevice));
    ...
    err = cudaGetLastError();
    if (err != cudaSuccess) {
      std::cout << "CUDA error (transport particle table): "
                << cudaGetErrorString(err) << std::endl;
      exit(-1);
    }
    gpu_err_chk(cudaMemcpy(tracked_particles, d_tracked_particles,
                           sizeof(tracked_particles), cudaMemcpyDeviceToHost));
...
//            std::cout << "Number of particles tracked " <<
//            tracked_particles[0] << std::endl;
#else
    mc::mc_world = this->world;
    mc::mc_vertices = this->vertices;
    worker_threads = new mqi::thrd_t[1];
    mc::transport_particles_table<R>(worker_threads, mc::mc_world,
                                     mc::mc_vertices, h1, tracked_particles);
#endif
    std::cout << "Number of particles tracked " << tracked_particles[0]
              << std::endl;
