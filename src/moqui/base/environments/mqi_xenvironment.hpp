...
#include "../mqi_fippel_physics.hpp"
#include "../mqi_io.hpp"
...
///< A virtual class to construct MC environment, e.g., geometries, scorers,
///< primaries
template <typename R> class x_environment {
...
public:
  ...
  virtual void initialize() {
    ...
    std::cout << "-------------------------------------------------------------"
                 "--------------"
              << std::endl;
    std::cout << "Starting process of creating world geometry and beam models.."
              << std::endl;
    std::cout << "-------------------------------------------------------------"
                 "--------------"
              << std::endl;
    ...
    std::cout << "-------------------------------------------------------------"
                 "--------------"
              << std::endl;
    std::cout << "Starting process of creating and uploading node to GPU.."
              << std::endl;
    std::cout << "-------------------------------------------------------------"
                 "--------------"
              << std::endl;
  ...
  void save_reshaped_files() {
      ...
      for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers;
        ...
        if (!this->output_format.compare("mhd")) {
          mqi::io::save_to_mhd<R>(this->world->children[c_ind], reshaped_data,
                                  1.0, this->output_path, filename, vol_size);
        } else if (!this->output_format.compare("mha")) {
          mqi::io::save_to_mha<R>(this->world->children[c_ind], reshaped_data,
                                  1.0, this->output_path, filename, vol_size);
        } else {
          mqi::io::save_to_bin<double>(reshaped_data, 1.0, this->output_path,
                                       filename, vol_size);
        }
    ...
    // std::milli> duration = stop - start; printf("Reshape and save done %f
    // ms\n", duration.count());
    std::cout << "Simulation result file reshape and saving process complete!"
              << std::endl;
  }
    ...
    for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
      for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers;
        ...
        mqi::io::save_to_npz<R>(this->world->children[c_ind]->scorers[s_ind],
                                1.0, this->output_path, filename, dim,
                                this->num_spots);
#else
        mqi::io::save_to_npz<R>(this->world->children[c_ind]->scorers[s_ind],
                                1.0, this->output_path, filename, dim,
...
#if defined(__CUDACC__)
    mqi::io::save_to_bin<R>(phantom->scorers[0], 1.0, this->output_path,
                            "/table_gpu_test");
#else
    mqi::io::save_to_bin<R>(phantom->scorers[0], 1.0, this->output_path,
                            "/table_cpu_test");
  ...
  void calculate_memory_expectation() {
    ...
    for (int node_ind = 0; node_ind < this->world->n_children; node_ind++) {
      ...
      mem += sizeof(mqi::scorer<R> **) *
             this->world->children[node_ind]->n_scorers;
      for (int s_ind = 0; s_ind < this->world->children[node_ind]->n_scorers;
           s_ind++) {
        mem += sizeof(R);
        mem += sizeof(uint32_t *) *
