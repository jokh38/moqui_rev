#ifndef MQI_APERTURE_HPP
#define MQI_APERTURE_HPP
...
public:
  ...
  aperture(std::vector<std::vector<std::array<float, 2>>> xypts,
           mqi::vec3<float> &v, mqi::vec3<float> &p, mqi::mat3x3<float> &r,
           bool is_rect = true)
      : block_data(xypts), volume(v), is_rectangle(is_rect),
  ...
  aperture(const mqi::aperture &rhs)
      : volume(rhs.volume), block_data(rhs.block_data),
