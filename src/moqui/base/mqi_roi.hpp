...
/// roi my ;
/// my.idx(v); -> c
//  if roi: length ==1 start = 0, stride = nb_transport_grid, acc_stride ==
//  stride
class roi_t {
public:
  ...
  ///< Following array pointers are set externally once all data uploaded via
  ///< cudaMemcpy
...
public:
  CUDA_HOST_DEVICE
  roi_t(roi_mapping_t m, uint32_t n, int32_t l = 0, uint32_t *s = nullptr,
        uint32_t *t = nullptr, uint32_t *a = nullptr)
      : method_(m), original_length_(n), length_(l), start_(s), stride_(t),
        acc_stride_(a) {
    ;
  }
