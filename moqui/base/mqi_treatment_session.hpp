...
#include "mqi_beam_module_ion.hpp"
#include "mqi_ct.hpp"
#include "mqi_dataset.hpp"
...
template <typename T> class treatment_session {
protected:
...
public:
  ...
  ///
  /// \param file_for_tx_machine : RTPLAN, IONPLAN, RTRECORD, IONRECORD - file
  /// name Currently IONPLAN and IONRECORD are supported only.
  treatment_session(std::string file_for_tx_machine, // for beamline and source,
    ...
    if (!is_file_valid)
      throw std::runtime_error(
          "Invalid DICOM file is given to treatment_session.");
    ...
    default:
      throw std::runtime_error(
          "treatment_session does not supports given RTMODALITY");
    }
    ...
    ///< machine name is set
    // If there is not beam name in function, use DICOM tag (beam name) in the
    // file.
    if (!m_name.compare("")) {
  ...
  /// \note it takes itype_ member variables. caution, itype_ shouldn't be
  /// changed after creation. \note we have a branch for machines based on
  /// "string" comparison.
  ///  Looking for better way to determine during 'ideally' pre-processing.
  /// type_traits allows to branch the logic flow based on the type of
  /// variables.
  bool create_machine(std::string machine_name, std::string mc_code,
                      int gantryNum) {
    if (tx_machine_)
    ...
    std::cout << "Creating treatment machine model.. : Machine name --> "
              << model << std::endl;
    std::cout << "Creating treatment machine model.. : MC code version --> "
              << mc_code << std::endl;
    std::cout << "Creating treatment machine model.. : Creating SMC treatment "
                 "machine model.."
              << std::endl;
    ...
    //     "\n"; tx_machine_ = new mqi::pbs<T>(model); material_   =
    //     mqi::pbs_material_t<T>(); return true;
    // } else {
  ...
  int get_num_beams() {
    auto beam_sequence = (*mqi_ds_)(seq_tags_->at("beam"));
    /// TODO: we need to count 'treatment' beam only. drop or not to use 'setup'
    /// beam.
    int count = 0;
    for (const auto &beam : beam_sequence) {
      count += 1;
  ...
  /// Search and return a beam (DICOM dataset) in BeamSequence for given beam
  /// name \param bnm for beam name \return mqi::dataset* constant pointer.
  ...
  /// Search and return a beam (DICOM dataset) in BeamSequence for given beam
  /// number \param bnm for beam number \return mqi::dataset* constant pointer.
  ...
  template <typename S> mqi::beamline<T> get_beamline(S beam_id) {
    if (mtype_ == 1)
      std::cout << "Creating beam line object.. : RT Plan type --> RT ION"
                << std::endl;
    std::cout << "Creating beam line object.. : Beam ID --> " << beam_id
              << std::endl;
    return tx_machine_->create_beamline(this->get_beam_dataset(beam_id),
                                        mtype_);
  }
  ...
  /// \param scale   for calculating number of histories to be simulated from
  /// beamlet weight \return beamsource object.
  template <typename S>
  mqi::beamsource<T> get_beamsource(S beam_id,
                                    const mqi::coordinate_transform<T> coord,
                                    float scale, T sid) {
    return tx_machine_->create_beamsource(this->get_beam_dataset(beam_id),
                                          mtype_, coord, scale, sid);
  }
  ...
  /// \param scale   for calculating number of histories to be simulated from
  /// beamlet weight \return beamsource object.
  mqi::beamsource<T> get_beamsource(mqi::dataset *beam,
                                    const mqi::coordinate_transform<T> coord,
                                    float scale, T sid) {
  ...
  /// \param beam_id
  template <typename S> std::map<T, int32_t> get_timeline(S beam_id) {
    return tx_machine_->create_timeline(this->get_beam_dataset(beam_id),
                                        mtype_);
  }

  /// Gets beam coordinate object for given beam id, e.g, beam name or beam
  /// number \param beam_id for beam number or beam name \return beam coordinate
  template <typename S> mqi::coordinate_transform<T> get_coordinate(S beam_id) {
    return tx_machine_->create_coordinate_transform(
        this->get_beam_dataset(beam_id), mtype_);
  }
  ...
  const gdcm::DataElement &get_dataelement(gdcm::Tag tag) {
