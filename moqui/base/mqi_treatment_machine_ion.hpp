#ifndef MQI_TREATMENT_MACHINE_ION_HPP
#define MQI_TREATMENT_MACHINE_ION_HPP
...
public:
  ...
  create_coordinate_transform(const mqi::dataset *ds,
    ...
    ///< As CW, CRW, NONE, conditionally required, we don't use them at this
    ///< moment
  ...
  ///        -1 means any of beamlet generates one history (usefule for
  ///        debugging).
  ...
  create_beamsource(const mqi::dataset *ds, const mqi::modality_type m,
    ...
    for (size_t i = 0; i < nb_spots; ++i) {
      ...
      beamsource.append_beamlet(this->characterize_beamlet(spots[i]),
                                nb_histories, pcoord, time_on_off[0],
                                time_on_off[1]);
    }
  ...
  /// \param m  modality type, e.g., RTIP or RTIBTR
  /// \param pcoord  coordinate transform to map global coordinate system
  /// \param particles_per_history a downscale factor, i.e., particles per
  /// history.
  ///        -1 means any of beamlet generates one history (usefule for
  ///        debugging).
  /// \param source_to_isocenter_mm distance the histories are generated.
  ...
  create_beamsource(const std::vector<mqi::beam_module_ion::spot> &spots,
    ...
    for (size_t i = 0; i < nb_spots; ++i) {
      ...
      //// TODO: history counting is different from the other method and would
      ///be very confisuing
      size_t nb_histories =
          (particles_per_history <= 1)
              ? 1
  ...
  virtual size_t characterize_history(const mqi::beam_module_ion::spot &s,
                                      float scale) {
    return s.meterset / scale;
  }

  /// User method to characterize number of histories per UNIFORM/MODULATED_SPEC
  /// beamlet based on spot information from DICOM.
  ...
  /// \note it's not yet clear whether to include coordinate_transform like
  /// create_beamsource \note currently we consider range-shiter and aperture.
  /// geometry will be added later.
  virtual mqi::beamline<T> create_beamline(const mqi::dataset *ds,
                                           mqi::modality_type m) {
    ...
    std::cout
        << "Creating beam line object.. : Number of range shifter sequence --> "
        << itmp[0] << std::endl;
    if (itmp[0] >= 1) {
    ...
    std::cout << "Creating beam line object.. : Number of blocks --> "
              << itmp[0] << std::endl;
    if (itmp[0] >= 1) {
  ...
  /// \param ds one-item of IonBeamSequence
  /// \param m  modality type, e.g., RTIP or RTIBTR
  /// \note Users need to implement their own rule to convert DICOM info to RS
  /// specification.
  ...
  /// \note Users need to implement their own rule to convert DICOM info to RS
  /// specification.
  virtual mqi::aperture *characterize_aperture(const mqi::dataset *ds,
                                               mqi::modality_type m) = 0;

  /// Returns set of (x,y) points for aperture openning.
  /// \param ds one-item of IonBeamSequence
  /// \param m  modality type, e.g., RTIP or RTIBTR
  /// \note Users need to implement their own rule to convert DICOM info to APT
  ...
  virtual mqi::vec3<T> beam_starting_position(const mqi::vec3<T> &iso, T z) {
    ...
    // std::cout << "Beam starting position : X --> " << beam.x << " Y --> " <<
    // beam.y << " Z --> " << beam.z << std::endl;
