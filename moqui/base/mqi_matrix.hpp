#ifndef MQI_MATRIX_HPP
#define MQI_MATRIX_HPP
...
public:
  ...
  CUDA_HOST_DEVICE
  mat3x3()
      : x(0), y(0), z(0), xx(1.0), xy(0), xz(0), yx(0), yy(1.0), yz(0), zx(0),
        zy(0), zz(1.0) {
    ;
  }
  ...
  mat3x3(T xx, T xy, T xz, T yx, T yy, T yz, T zx, T zy, T zz)
      : x(0), y(0), z(0), xx(xx), xy(xy), xz(xz), yx(yx), yy(yy), yz(yz),
        zx(zx), zy(zy), zz(zz) {
    ;
  }
  ...
  CUDA_HOST_DEVICE
  mat3x3(T a, T b, T c)
      : x(a), y(b), z(c), xx(1.0), xy(0), xz(0), yx(0), yy(1.0), yz(0), zx(0),
        zy(0), zz(1.0) {
  ...
  mat3x3(std::array<T, 3> &abc)
      : x(abc[0]), y(abc[1]), z(abc[2]), xx(1.0), xy(0), xz(0), yx(0), yy(1.0),
        yz(0), zx(0), zy(0), zz(1.0) {
    if (x != 0)
      this->rotate_x(x);
    if (y != 0)
      this->rotate_y(y);
  ...
  mat3x3<T> operator*(const mat3x3<T> &r) const {
    return mat3x3<T>(
        xx * r.xx + xy * r.yx + xz * r.zx, xx * r.xy + xy * r.yy + xz * r.zy,
...
#else
    std::cout << "(xx, xy, xz) -> (" << xx << " " << xy << " " << xz << ")"
              << std::endl;
    std::cout << "(yx, yy, yz) -> (" << yx << " " << yy << " " << yz << ")"
              << std::endl;
    std::cout << "(zx, zy, zz) -> (" << zx << " " << zy << " " << zz << ")"
              << std::endl;
#endif
  }
};
...
public:
  ...
  CUDA_HOST_DEVICE
  mat4x4()
      : xx(1.0), xy(0), xz(0), xs(0), yx(0), yy(1.0), yz(0), ys(0), zx(0),
        zy(0), zz(1.0), zs(0), sx(0), sy(0), sz(0.0), ss(1.0) {
    ;
  }

  CUDA_HOST_DEVICE
  mat4x4(T xx, T xy, T xz, T xs, T yx, T yy, T yz, T ys, T zx, T zy, T zz, T zs,
         T sx, T sy, T sz, T ss)
      : xx(xx), xy(xy), xz(xz), xs(xs), yx(yx), yy(yy), yz(yz), ys(ys), zx(zx),
        zy(zy), zz(zz), zs(zs), sx(sx), sy(sy), sz(sz), ss(ss) {
    ;
  }
...
#else
    std::cout << "(xx, xy, xz, xs) -> (" << xx << ", " << xy << ", " << xz
              << ", " << xs << ")" << std::endl;
    std::cout << "(yx, yy, yz, ys) -> (" << yx << ", " << yy << ", " << yz
              << ", " << ys << ")" << std::endl;
    std::cout << "(zx, zy, zz, zs) -> (" << zx << ", " << zy << ", " << zz
              << ", " << zs << ")" << std::endl;
    std::cout << "(zx, zy, zz, ss) -> (" << sx << ", " << sy << ", " << sz
              << ", " << ss << ")" << std::endl;
#endif
  }
};
