...
#include "mqi_rect3d.hpp"

namespace mqi {

/// \class ct
/// this ct class is a int16_t pixel type.
/// \tparam R for grid coordinate (float or double) a type of x, y, and z
/// positions
template <typename R> class ct : public rect3d<int16_t, R> {
protected:
  std::vector<std::string>
      files_; ///<  DICOM image files sorted in Z acsending order

  std::map<std::string, std::string>
      uid2file_; ///< SOP instance UID to id in files_
...
public:
  ...
  ct(std::string f, bool is_print = false) {
    ...
    for (size_t i = 0; i < nz; ++i) {
      ...
      rect3d<int16_t, R>::z_[i] =
          (R)(std::stod(img_position.substr(deli1 + 1)));
      ...
      uid2file_.insert(std::make_pair(
          m0.find(gdcm::Tag(0x0008, 0x0018))->second, files_[i]));
    }
    ...
    std::cout << "Reading DICOM directory.. : Getting patient CT info.."
              << std::endl;
    std::cout << "Reading DICOM directory.. : Detected CT info is as below."
              << std::endl;
    std::cout << "CT (nx, ny, nz) -> (" << rect3d<int16_t, R>::dim_.x << ", "
              << rect3d<int16_t, R>::dim_.y << ", "
              << rect3d<int16_t, R>::dim_.z << ")" << std::endl;
    std::cout << "CT (dx, dy, dz) -> (" << dx_ << ", " << dy_ << ", "
              << rect3d<int16_t, R>::z_[1] - rect3d<int16_t, R>::z_[0] << ")"
              << std::endl;
  ...
  virtual void load_data() {
    std::cout << "Reading DICOM directory.. : Loading patient CT pixel data.."
              << std::endl;
    size_t nb_voxels_2d =
        rect3d<int16_t, R>::dim_.x * rect3d<int16_t, R>::dim_.y;
    ...
    rect3d<int16_t, R>::data_ =
        rect3d<int16_t, R>::data_ * int16_t(slope) + intercept;
    std::cout << "Reading DICOM directory.. : Patient CT pixel data "
                 "successfully loaded."
              << std::endl;
  }
