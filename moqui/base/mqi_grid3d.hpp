#ifndef MQI_GRID3D_HPP
#define MQI_GRID3D_HPP
...
#include "mqi_coordinate_transform.hpp"
...
template <typename R> struct intersect_t {
...
template <typename T, typename R> class grid3d {
...
public:
  ...
  CUDA_HOST_DEVICE
  grid3d(const R xe[], const ijk_t n_xe, const R ye[], const ijk_t n_ye,
         const R ze[], const ijk_t n_ze) {
    xe_ = new R[n_xe];
    ye_ = new R[n_ye];
    ze_ = new R[n_ze];
  ...
  grid3d(const R xe_min, const R xe_max,
         ...
         const R ye_min, const R ye_max, const ijk_t n_ye, const R ze_min,
         const R ze_max, const ijk_t n_ze) {
    xe_ = new R[n_xe];
    ye_ = new R[n_ye];
    ze_ = new R[n_ze];
  ...
  /// Construct a rectlinear grid from array of x/y/z with their size and
  /// rotation angless \param x,y,z  1D array of central points of voxels along
  /// x-axis \param xn,yn,zn  size of 1D array for points. \parmas angles
  /// rotation angle in degree for each axis.
  CUDA_HOST_DEVICE
  grid3d(const R xe_min, const R xe_max,
         const ijk_t n_xe, // n_xe : steps + 1
         const R ye_min, const R ye_max, const ijk_t n_ye, const R ze_min,
         const R ze_max, const ijk_t n_ze, std::array<R, 3> &angles) {
    xe_ = new R[n_xe];
    ye_ = new R[n_ye];
    ze_ = new R[n_ze];
  ...
  grid3d(const R xe_min, const R xe_max,
         const ijk_t n_xe, // n_xe : steps + 1
         const R ye_min, const R ye_max, const ijk_t n_ye, const R ze_min,
         const R ze_max, const ijk_t n_ze, mqi::mat3x3<R> rxyz) {
    xe_ = new R[n_xe];
    ye_ = new R[n_ye];
    ze_ = new R[n_ze];
  ...
  CUDA_HOST_DEVICE
  grid3d(const R xe[], const ijk_t n_xe, const R ye[], const ijk_t n_ye,
         const R ze[], const ijk_t n_ze, mqi::mat3x3<R> rxyz) {
    xe_ = new R[n_xe];
    ye_ = new R[n_ye];
    ze_ = new R[n_ze];
  ...
  CUDA_HOST_DEVICE
  grid3d(const R xe[], const ijk_t n_xe, const R ye[], const ijk_t n_ye,
         const R ze[], const ijk_t n_ze, std::array<R, 3> &angles) {
    xe_ = new R[n_xe];
    ye_ = new R[n_ye];
    ze_ = new R[n_ze];
  ...
  CUDA_HOST_DEVICE
  virtual const T operator[](const mqi::vec3<ijk_t> p) {
  ...
  /// Reads data from other source,  currently values are sum of index square
  /// for testing
  ...
  intersect_t<R> intersect(mqi::vec3<R> &p, mqi::vec3<R> &d,
    ...
    R pe = p.dot(n100_);
    /// non intersect
    /// Is this required? Since the particle is alreay in a voxel, it should
    /// intersect with some surface
    if (me * me > mqi::near_zero) {
      if (me < 0) { // if intersect, X-
        if (mqi::mqi_abs(-(p.x - vox1.x) / d.x) < mqi::geometry_tolerance &&
            idx.x > 0) {
    ...
    if (me * me > mqi::near_zero) {
      if (me < 0) { // if intersect, Y-
        if (mqi::mqi_abs(-(p.y - vox1.y) / d.y) < mqi::geometry_tolerance &&
            idx.y > 0) {
    ...
    if (me * me > mqi::near_zero) {
      if (me < 0) { // if intersect, Z-
        if (mqi::mqi_abs(-(p.z - vox1.z) / d.z) < mqi::geometry_tolerance &&
            idx.z > 0) {
  ...
  intersect_t<R> intersect(mqi::vec3<R> &p, mqi::vec3<R> &d) {
    mqi::intersect_t<R> its; // return value
    if (p.x >= xe_[0] && p.x <= xe_[dim_.x] && p.y >= ye_[0] &&
        p.y <= ye_[dim_.y] && p.z >= ze_[0] && p.z <= ze_[dim_.z]) {
...
#endif
    if ((u_min < u_max || std::abs(u_min - u_max) < mqi::geometry_tolerance) &&
        u_min >= 0 && u_max >= 0) {
      its.dist = u_min;
  ...
  CUDA_HOST_DEVICE
  inline mqi::vec3<ijk_t> index(const mqi::vec3<R> &p,
                                mqi::vec3<R> &dir) // if p is on boundary
  ...
  CUDA_HOST_DEVICE
  inline void index(mqi::vec3<R> &vtx1, mqi::vec3<R> &dir1,
                    mqi::vec3<ijk_t> &idx) {
    // find intersection for the rest of the intersection, retun voxel index
    /// Do we need to deal with exceptions? Like, p.x-xe_[idx.x]<dx which must
    /// not happend

    int flag_x = 0, flag_y = 0, flag_z = 0;
    R x_edge1, x_edge2;
    if (dir1.x < 0 &&
        (mqi::mqi_abs(vtx1.x - xe_[idx.x]) < mqi::geometry_tolerance ||
         vtx1.x < xe_[idx.x])) {
      idx.x -= 1;
    } else if (dir1.x > 0 && (mqi::mqi_abs(vtx1.x - xe_[idx.x + 1]) <
                                  mqi::geometry_tolerance ||
                              vtx1.x > xe_[idx.x + 1])) {
      idx.x += 1;
    }

    if (dir1.y < 0 &&
        (mqi::mqi_abs(vtx1.y - ye_[idx.y]) < mqi::geometry_tolerance ||
         vtx1.y < ye_[idx.y])) {
      idx.y -= 1;
    } else if (dir1.y > 0 && (mqi::mqi_abs(vtx1.y - ye_[idx.y + 1]) <
                                  mqi::geometry_tolerance ||
                              vtx1.y > ye_[idx.y + 1])) {
      idx.y += 1;
    }
    ...
    if (dir1.z < 0 &&
        (mqi::mqi_abs(vtx1.z - ze_[idx.z]) < mqi::geometry_tolerance ||
         vtx1.z < ze_[idx.z])) {
      idx.z -= 1;
    } else if (dir1.z > 0 && (mqi::mqi_abs(vtx1.z - ze_[idx.z + 1]) <
                                  mqi::geometry_tolerance ||
                              vtx1.z > ze_[idx.z + 1])) {
      idx.z += 1;
    }
  ...
  inline bool is_valid(mqi::vec3<ijk_t> &c) {
    ...
    if (c.x >= dim_.x || c.y >= dim_.y || c.z >= dim_.z)
      return false;
    //            if (c.x > dim_.x || c.y > dim_.y || c.z > dim_.z) return
    //            false;
