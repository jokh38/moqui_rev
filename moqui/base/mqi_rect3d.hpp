#ifndef MQI_RECT3D_HPP
#define MQI_RECT3D_HPP
...
protected:
  ...
  // To make navigation and interpolation problems simple, we assume Transport
  // matrix X: 1, 0, 0 Y: 0, 1, 0 Z: 0, 0, 1 or set one of following flag
  bool flip_[3] = {false, false, false};

  // Edge data is not neccessary for 4D interpolation but important for
  // navigation R  x_edge[2]; /**< x boundary of entire rectlinear, e.g.,
  // x_edge[0] for x min, x_edge[1] for x max*/ R  y_edge[2]; /**< y boundary of
  // entire rectlinear, e.g., y_edge[0] for y min, y_edge[1] for y max*/ R
  // z_edge[2]; /**< y boundary of entire rectlinear, e.g., z_edge[0] for z min,
  // z_edge[1] for z max*/
...
public:
  ...
  /// \param p is a std::array to represent a position, p[0], p[1], p[2] for x,
  /// y, z.
  ...
  /// Returns a coner index (x,y,z) where a given position is placed within
  /// (x+1,y+1,z+1) \param pos cartesian coordinate of x, y, and z. \return
  /// x,y,z indices of a cell
  CUDA_HOST
  ...
  inline virtual size_t find_c000_x_index(const R &x) {
    // In case this code runs on GPU, consider to implement binary_search
    // algorithm or use thrust but thrust performance is not so good from
  ...
  virtual void write_data(const std::string filename) {
    ...
    file1.write(reinterpret_cast<const char *>(&data_[0]),
                data_.size() * sizeof(T));
    file1.close();
  }
  ...
  void write_data(std::valarray<S> &output, const std::string filename) {
    std::ofstream file1(filename, std::ios::out | std::ofstream::binary);
    file1.write(reinterpret_cast<const char *>(&output[0]),
  ...
  vtkSmartPointer<vtkMetaImageWriter>::New();
      ...
      std::ofstream file1( filename, std::ios::out | std::ofstream::binary);
      file1.write(reinterpret_cast<const char *>(&data_[0]), data_.size() *
  sizeof(T));r file1.close();
  ...
  */
    ...
    for (int k = 0; k < dim_.z; ++k) {
      for (int j = 0; j < dim_.y; ++j) {
        for (int i = 0; i < dim_.x; ++i) {
          ...
          idz = (flip_[2]) ? (dim_.z - 1 - k) * dim_.x * dim_.y
                           : k * dim_.x * dim_.y;
  ...
  template <typename T0, typename R0, typename T1, typename R1>
  friend void interpolate(rect3d<T0, R0> &src, rect3d<T1, R1> &dest,
                          T1 &fill_value);

  /// A friend function to warp source data (rect3d) to destination (rect3d)
  /// using dvf. It will pull src data from ref thus DVF of ref->src is
  /// neccessary. In MIM, when we calculate DIR, ref should be chosen first. It
  /// is recommended that src, dest, dvf have all same resolution.
...
void interpolate(rect3d<T0, R0> &src, rect3d<T1, R1> &dest, T1 &fill_value) {
  ...
  std::cout << dest.x_[0] << ", " << dest.y_[0] << ", " << dest.z_[0]
            << std::endl;
...
template <typename T0, typename R0, typename S0>
void warp_linear(rect3d<T0, R0> &src, rect3d<T0, R0> &dest,
                 rect3d<mqi::vec3<S0>, R0> &vf, T0 fill_value) {
  ///< Number of voxels: Nx, Ny, Nz
  const size_t nX = dest.dim_.x;
  const size_t nY = dest.dim_.y;
  const size_t nZ = dest.dim_.z;
    ...
    for (size_t j = 0; j < nY; ++j) {
      ...
      for (size_t i = 0; i < nX; ++i) {
        ...
        if (vf.is_in_point(p_dest)) {
          ...
          /// Then, assign value by interpolating values at 8 coners surrounding
          /// new position.
          ///
