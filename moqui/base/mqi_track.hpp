...
///< KILLED  : no further tracking is required due to energy 0 or exit world
///< boundary?
typedef enum {
  CREATED = 0, ///< created
...
template <typename R> class track_t {
public:
  uint32_t scorer_column; ///< id of beamlet or time-index can be used for Dij
                          ///< or Dit matrix
  ...
  CUDA_HOST_DEVICE
  track_t()
      : status(CREATED), process(BEGIN), primary(true), dE(0), scorer_column(0),
        local_dE(0) {
    ;
  }
  ...
  ///< Constructor
  CUDA_HOST_DEVICE
  track_t(status_t s, process_t p, bool is_p, particle_t t, vertex_t<R> v0,
          vertex_t<R> v1, const R &dE)
      : status(s), process(p), primary(is_p), particle(t), vtx0(v0), vtx1(v1),
        dE(dE), scorer_column(0), local_dE(0) {
    ;
  }
  ...
  void update_post_vertex_direction(const R &theta, const R &phi) {
    ...
    mqi::vec3<R> d_local =
        m_local * ref_vector; // rotate about the z-axis (dir)
    d_local.normalize();
  ...
  CUDA_HOST_DEVICE
  void update_post_vertex_position(const R &len) {
