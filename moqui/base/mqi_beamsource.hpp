#ifndef MQI_BEAMSOURCE_HPP
#define MQI_BEAMSOURCE_HPP

/// \file
///
/// A beamsource is a collection of beamlets and provides an interface for
/// sampling.
...
/// \note
template <typename T> class beamsource {
public:
  /// A beamlet container.
  /// Each element is a tuple of beamlet, number of histories, and accumlated
  /// histories
  std::vector<std::tuple<mqi::beamlet<T>, size_t, size_t>> beamlets_;

  /// A lookup table to map a history to beamlet id.
  /// note: To run GPU, this std::map needs to be replaced GPU compatible data
  /// structure.
  ...
  void append_beamlet(mqi::beamlet<T> b, size_t h,
                      mqi::coordinate_transform<T> p, T time_on = 1,
                      T time_off = 0) {
  ...
  void append_beamlet_log(mqi::beamlet<T> b, size_t h,
  ...
  /// \param b a beamlet
  /// \param h number of histories
  void append_beamlet(mqi::beamlet<T> b, size_t h, T time_on = 1,
                      T time_off = 0) {
    const size_t acc = total_histories() + h;
    const size_t beamlet_id =
        this->total_beamlets(); // current number of beamlets -> beamlet ID
  ...
  size_t cumulative_history_at_time(T t0) {
    ...
    T pulse_time[2];
    pulse_time[0] =
        (pulse == timeline_.begin()) ? 0.0 : std::prev(pulse)->first;
