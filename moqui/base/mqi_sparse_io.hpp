...
void save_npz(std::string filename, std::string var_name, std::string data,
              size_t shape, std::string mode);

template <typename T>
void save_npz(std::string filename, std::string var_name, T *data, size_t shape,
              std::string mode);
...
void parse_zip_footer(std::string filename, uint16_t &nrecs,
...
void mqi::io::parse_zip_footer(std::string filename, uint16_t &nrecs,
...
void mqi::io::save_npz(std::string filename, std::string var_name,
                       std::string data, size_t shape, std::string mode) {
  std::fstream fid_out;
  uint16_t nrecs = 0;
  ...
  if (mode == "a") {
    size_t global_header_size;
    mqi::io::parse_zip_footer(filename, nrecs, global_header_size,
                              global_header_offset);
    fid_out.open(filename, std::ios::in | std::ios::out | std::ios::binary |
                               std::ios::ate);
    ...
    if (!fid_out) {
      std::cout << "error: only " << fid_out.gcount() << " could be read"
                << std::endl;
      exit(-1);
    }
    fid_out.seekg(global_header_offset, fid_out.beg);
  ...
  crc =
      crc32(crc, (uint8_t *)data.c_str(), sizeof(std::string) * data.length());
  ...
  mqi::io::push_value(global_header,
  ...
  fid_out.write(reinterpret_cast<const char *>(&header[0]),
                sizeof(char) * header.size());
  fid_out.write(reinterpret_cast<const char *>(&data[0]),
                sizeof(std::string) * data.length());
  fid_out.write(reinterpret_cast<const char *>(&global_header[0]),
                sizeof(char) * global_header.size());
  fid_out.write(reinterpret_cast<const char *>(&footer[0]),
                sizeof(char) * footer.size());
  fid_out.close();
}

template <typename T>
void mqi::io::save_npz(std::string filename, std::string var_name, T *data,
                       size_t shape, std::string mode) {
  std::fstream fid_out;
  uint16_t nrecs = 0;
  size_t global_header_offset = 0;
