...
#include "material_table_data.hpp"
...
///<
template <typename R> class material_t {
public:
  physics_constants<R> units; ///< TODO: Consider moving to shared physics
                              ///< constants system for better memory efficiency
...
public:
  ...
  virtual R radiation_length() {
    ...
    radiation_length_mat =
        (this->units.water_density * this->units.radiation_length_water) /
        (density_tmp * 1e-3 * f);
...
///< water_t
template <typename R> class h2o_t : public material_t<R> {
public:
  CUDA_HOST_DEVICE
  h2o_t() : material_t<R>() {
...
///< air_t
template <typename R> class air_t : public material_t<R> {
public:
  CUDA_HOST_DEVICE
  air_t() : material_t<R>() {
    ///< TODO: Implement accurate air composition values (currently using
    ///< placeholders)
    ...
    this->two_pi_re2_mc2_nel =
        material_t<R>::units.two_pi_re2_mc2_h2o * this->rho_mass;
    this->Iev = 85.7;
    this->Iev_sq = 85.7 * 85.7;
    this->Z = 18;
    this->X0 =
        (36.62 * this->units.cm) * this->rho_mass; // 36.0863/this->units.cm;
  }
...
///< brass_t
template <typename R> class brass_t : public material_t<R> {
public:
  CUDA_HOST_DEVICE
