...
#include "../mqi_file_handler.hpp"
#include "../mqi_io.hpp"
#include "../mqi_math.hpp"
#include "../mqi_rangeshifter.hpp"
#include "../mqi_roi.hpp"
#include "../mqi_threads.hpp"
#include "../mqi_treatment_session.hpp"
...
template <typename R> class tps_env : public x_environment<R> {
public:
  ...
  bool usingPhantomGeo =
      false; // to use phantom environment, added in 2024-04-11
...
public:
  ...
  tps_env(const std::string input_name) : x_environment<R>() {
    std::cout << "-------------------------------------------------------------"
                 "-------------------"
              << std::endl;
    std::cout << "MOQUI SMC Treatment machine model and log-file based MC"
              << std::endl;
    std::cout << "Modified by Chanil Jeon of Sungkyunkwan University in 2024"
              << std::endl;
    ...
    std::cout << "Reading GPU ID.. : Selected GPU ID --> " << this->gpu_id
              << std::endl;
#if defined(__CUDACC__)
    cudaSetDevice(this->gpu_id);
#endif
    master_seed = parser.get_int("RandomSeed", -1);
    std::cout << "Reading seeds.. : Inserted seed --> " << master_seed
              << std::endl;
    if (master_seed < -1)
      std::cout << "Reading seeds.. : Using negative value generates current "
    ...
    //        std::string aperture_string = parser.get_string("ApertureType",
    //        "VOLUME"); aperture_type           =
    //        parser.string_to_aperture_type(aperture_string);
    ...
    if (use_absolute_path) {
      ...
      this->parent_dir = parser.get_string("ParentDir", "");
      this->dicom_dir =
          this->parent_dir + "/" + parser.get_string("DicomDir", "");
      this->logfile_dir =
          this->parent_dir + "/" +
          parser.get_string(
              "logFilePath",
    ...
    std::cout << "Selected gantry number : " << this->selectedGantryNumber
              << std::endl;
    ...
    // ---------------------------------------------------------------------------------------------
    if (!parent_dir.empty())
      std::cout << "Reading patient directory.. : Patient directory name --> "
                << parent_dir << std::endl;
    ...
    sim_type = parser.string_to_sim_type(
        parser.get_string("SimulationType", "perBeam"));
    particles_per_history = parser.get_float("ParticlesPerHistory", -1.0);

    // -------------------------------------------------------------------------------------------
    /// Scorer parameters
    this->scorer_string = parser.get_string(
        "Scorer", "EnergyDeposition"); // assumes only one scorer
    ...
    if (mask_filenames.size() > 0)
      ...
      std::cout << "Reading masking file.. : There is no masking file to read."
                << std::endl;
    ...
    if (output_path.empty()) {
      ...
      std::cout << "Setting output.. : Output directory name --> "
                << output_path << std::endl;
      std::cout << "Setting output.. : Output format --> " << output_format
                << std::endl;
    }
    ...
    tx = new mqi::treatment_session<R>(dcm_.plan_name, machineName,
                                       referenceMCName,
  ...
  virtual struct dicom_t read_dcm_dir() {
    ...
    std::cout << "Reading DICOM directory.. : DICOM directory name --> "
              << dicom_dir << std::endl;
    ...
    if (dcm.n_plan > 0) {
      if (dcm.n_plan > 1)
        printf("There are multiple RTPLANs in the path. Selecting the first "
               "one\n");
    ...
    if (dcm.n_struct > 0) {
      if (dcm.n_struct > 1)
        printf("There are multiple RTSTRUCTs in the path. Selecting the first "
               "one\n");
    ...
    std::cout << "Reading DICOM directory.. : Loading RT Ion plan data from "
              << dcm.plan_name << " .." << std::endl;

    if (!this->usingPhantomGeo) {
      if (dcm.n_plan > dcm.nfiles || dcm.n_dose > dcm.nfiles ||
          dcm.n_struct > dcm.nfiles || dcm.dim_.z > dcm.nfiles) {
    ...
    if (!this->usingPhantomGeo) {
      ...
      for (int i = 0; i < dcm.dim_.z + 1; i++) {
        if (i == 0)
          ...
          dcm.org_ze[i] = dcm.org_ze[i - 1] +
                          dcm.org_dz[i - 1]; // need to be modified to deal with
                                             // varying slice thickness
      }

      dcm.image_center.x = (dcm.org_xe[0] + dcm.dx / 2.0 +
                            dcm.org_xe[dcm.dim_.x] - dcm.dx / 2.0) /
                           2.0;
      dcm.image_center.y = (dcm.org_ye[0] + dcm.dy / 2.0 +
                            dcm.org_ye[dcm.dim_.y] - dcm.dy / 2.0) /
                           2.0;
      dcm.image_center.z =
          (dcm.org_ze[0] + dcm.org_dz[0] / 2.0 + dcm.org_ze[dcm.dim_.z] -
      ...
      dcm.dz = dcm.org_dz;
      this->ct_data =
          new int16_t[dcm.org_dim_.x * dcm.org_dim_.y * dcm.org_dim_.z];
      std::copy(begin(dcm.ct->get_data()), end(dcm.ct->get_data()),
                this->ct_data);
    } else {
      // array of the dcm.ct contains center of voxels
    ...
    if (dcm.n_struct >= 1 && this->read_structure) {
      ...
      mqi::dataset *struct_ds_ =
          new mqi::dataset(struct_reader.GetFile().GetDataSet(), true);
      auto roi_seq = (*struct_ds_)(gdcm::Tag(0x3006, 0x0020));
      printf("Reading ROI information from RT structure.. --> ROI sequence "
             "size : %lu\n",
             roi_seq.size());
      ...
      for (int roi_ind = 0; roi_ind < roi_seq.size(); roi_ind++) {
        ...
        if (strcasecmp(roi_name[0].c_str(), this->body_contour_name.c_str()) ==
            0) {
          printf(
              "Reading ROI information from RT structure.. --> ROI name : %s\n",
              roi_name[0].c_str());
      ...
      uint8_t *body_contour =
          new uint8_t[dcm.org_dim_.x * dcm.org_dim_.y * dcm.org_dim_.z];
      ...
      for (int con_ind = 0; con_ind < roi_contour_seq.size(); con_ind++) {
        ...
        if (refer_roi[0] == body_ind[0]) {
          auto contour_seq =
              (*roi_contour_seq[con_ind])(gdcm::Tag(0x3006, 0x0040));
          for (int contour_ind = 0; contour_ind < contour_seq.size();
               contour_ind++) {
            contour_seq[contour_ind]->get_values("NumberOfContourPoints",
                                                 contour_num);
            ...
            fill_contour(body_contour, contour_points, contour_num[0], dcm.dim_,
                         dcm.xe, dcm.ye, dcm.ze, dcm.dx, dcm.dy);
  ...
  virtual struct logfiles_t read_logfile_dir(int beamIndex) {
    ...
    for (int i = 0; i < fileFolderName.size(); i++) {
      if (i == beamIndex) {
        ...
        // Declare log file energy, spot XY, particle count information for each
        // field
        ...
        for (auto &filename : sortedByName) {
          ...
          if (filename.extension() == ".csv") {
            ...
            std::cout
                << "Reading log file information in the field directory.. : " +
                       filename.string() + ".."
                << std::endl;
  ...
  virtual void setup_world() {
    ...
    // When we use custom phantom geometry, add air geometry to child of the
    // world geometry Therefore, this varaiable gives additional size to
    // geometry container
    ...
    // Because the program don't calculate anything in world geometry, set
    // rho_mass to 0
    ...
    std::vector<mqi::geometry *> beamline_geometries =
        beamline.get_geometries();
    ...
    if (this->usingPhantomGeo) {
      ...
      const mqi::dataset *beamDataset =
          this->tx->get_beam_dataset(bnb); // Get beam dataset
      const mqi::dataset *icps = (*beamDataset)(
          "IonControlPointSequence")[0]; // Get isocenter position
      ...
      // Set air box to end index of beamline object, becuase the simulation
      ...
      airBox->geo = new grid3d<density_t, R>(
          -200, 200, 2, -200, 200, 2, airEndPos,
    ...
    for (int i = 0; i < beamline_geometries.size(); i++) {
      ...
      if (beamline_geometries[i]->geotype == mqi::RANGESHIFTER) {
        ...
        //                beamline_objects[i+1] =
        //                this->create_voxelixed_aperture(
    ...
    ///< create a child
    std::cout << "Creating child in world geometry.. : Phantom size -->"
              << std::endl;
    if (this->twoCentimeterMode)
      std::cout << "(x, y, z) -> (400, 400, 2)" << std::endl;
    else
      std::cout << "(x, y, z) -> (" << this->dcm_.dim_.x << ", "
                << this->dcm_.dim_.y << ", " << this->dcm_.dim_.z << ")"
                << std::endl;
    ...
    if (!this->usingPhantomGeo) {
      ...
      phantom->geo = new grid3d<density_t, R>(
          this->dcm_.xe, this->dcm_.dim_.x + 1, this->dcm_.ye,
          this->dcm_.dim_.y + 1, this->dcm_.ze, this->dcm_.dim_.z + 1);
      density_t *rho_mass =
          new density_t[dcm_.dim_.x * dcm_.dim_.y * dcm_.dim_.z];
      ...
      phantom->geo->set_data(
          rho_mass); //// Material conversion function required
    } else           // 2. If user uses phantom geometry
    {
      mqi::coordinate_transform<R> transformPhantom =
          this->tx->get_coordinate(bnb);
      ...
      if (this->twoCentimeterMode) {
        ...
        phantom->geo =
            new grid3d<density_t, R>(-200, 200, 401, -200, 200, 401, -1, 1, 2,
                                     transformPhantom.rotation);
    ...
    if (scoring_mask) {
      ...
      roi_tmp = new roi_t(mqi::DIRECT, this->dcm_.dim_.x * this->dcm_.dim_.y *
                                           this->dcm_.dim_.z);
    }
    ...
    mqi::key_value *deposit0 =
        new mqi::key_value[phantom->scorers[0]->max_capacity_];

    std::memset(deposit0, 0xff,
                sizeof(mqi::key_value) * phantom->scorers[0]->max_capacity_);
    ...
    if (this->score_variance) {
      ...
      for (int s_ind; s_ind < phantom->n_scorers; s_ind++) {
        count[s_ind] =
            new mqi::key_value[phantom->scorers[s_ind]->max_capacity_];
        vox_mean[s_ind] =
            new mqi::key_value[phantom->scorers[s_ind]->max_capacity_];
        vox_variance[s_ind] =
            new mqi::key_value[phantom->scorers[s_ind]->max_capacity_];
  ...
  virtual void setup_beamsource() {
    ...
    std::cout << "Creating beam source object.. : Detected beam count --> "
              << num_beams << std::endl;
    std::cout << "Creating beam source object.. : Currently selected beam --> "
              << bnb << " (" << beam_names[bnb - 1] << ")" << std::endl;

    const mqi::dataset *mqi_ds = this->tx->get_beam_dataset(bnb);
    const mqi::dataset *ion_beam_sequence =
        (*mqi_ds)("IonControlPointSequence")[0];
    std::vector<float> temp_sid;
    ion_beam_sequence->get_values("SnoutPosition", temp_sid);
    this->sid = temp_sid[0];
    std::cout << "Creating beam source object.. : Detected snout position --> "
              << this->sid << " mm" << std::endl;
    mqi::coordinate_transform<R> p_coord = this->tx->get_coordinate(bnb);
    p_coord.angles[3] = 90.0; // iec2dicom angle
    ...
    std::cout << "Creating coordinate transform for beam source complete.. : "
                 "p_coord angle information -- >"
              << std::endl;
    std::cout << "(" << p_coord.angles[0] << ", " << p_coord.angles[1] << ", "
              << p_coord.angles[2] << ", " << p_coord.angles[3] << ")"
              << std::endl;
    ...
    this->tx->get_beam_dataset(bnb)->get_values("NumberOfRangeShifters",
                                                rangeShifterCount);
    ...
    this->beamsource =
        this->tx->get_beamsource(logFileData, p_final, sid, rangeShifterUsed);
    // ****************************************************************************

    std::cout << "Total beamlets (Spots) : "
              << this->beamsource.total_beamlets() << std::endl;
    std::cout << "Total histories (Particles) : "
              << this->beamsource.total_histories() << std::endl;
  ...
  virtual void run() {
    ...
    if (this->sim_type == mqi::PER_BEAM) {
      ...
      // 1024.0));
      std::cout
          << "Starting simulation per field.. : Geometry allocated memory --> "
  ...
  CUDA_HOST
  virtual void run_simulation(size_t histories_per_batch,
    ...
    /// the histories_per_batch may not required if copying memory work
    /// correctly with histories_in_batch
    ...
    // std::milli> duration = stop - start; printf("Initialization for run done
    // %f ms\n", duration.count());
#if defined(__CUDACC__)
    // start     = std::chrono::high_resolution_clock::now();
    ...
    if (histories_in_batch < n_threads) {
      ...
      if (histories_in_batch % (n_threads * n_blocks * particles_per_thread) >
          0)
    ...
    printf("Printing simulation specification.. : Block size --> %d, Thread "
           "size --> %d\n",
           n_blocks, n_threads);
    mc::upload_vertices(this->vertices, mc::mc_vertices, 0,
                        histories_per_batch);
    ...
    if (scorer_offset_vector) {
      printf(
          "Printing simulation specification.. : Histories per batch --> %d\n",
          histories_per_batch);
    ...
    gpu_err_chk(cudaMalloc(&worker_threads,
                           n_blocks * n_threads * sizeof(mqi::thrd_t)));
    // start = std::chrono::high_resolution_clock::now();
    ...
    gpu_err_chk(cudaMemcpy(d_tracked_particles, tracked_particles,
    ...
    gpu_err_chk(cudaMemcpy(tracked_particles, d_tracked_particles,
                           sizeof(tracked_particles[0]),
    ...
    mc::transport_particles_patient<R>(worker_threads, mc::mc_world,
                                       mc::mc_vertices, histories_in_batch,
                                       tracked_particles);
#endif
  } // run_simulation
  ...
  void read_vertices_spot(size_t history_start, size_t history_end,
                          ...
                          uint32_t *score_offset_vector, int spot_ind,
                          size_t histories_per_batch) {
    for (int history_ind = history_start; history_ind < history_end;
         history_ind++) {
  ...
  virtual void run_by_beam(mqi::node_t<R> *world = mc::mc_world) {
    ...
    for (int batch = 0; batch < num_batches; batch++) {
      ...
      printf("Generating particles for (%d of %d batches) in CPU ..\n",
             batch + 1, num_batches);
      for (current_vertex = 0; current_vertex < histories_per_batch;
           current_vertex++) {
  ...
  virtual void run_by_spot(mqi::node_t<R> *world = mc::mc_world) {
    ...
    if (this->max_histories_per_batch == 0) {
      ...
      num_batches =
          (int)mqi::mqi_ceil(h1 * 1.0 / this->max_histories_per_batch);
      histories_per_batch = this->max_histories_per_batch;
      printf("Upload %lu histories per batch, %lu batches expected\n",
             histories_per_batch, num_batches);
    }
    ...
    while (spot_ind < this->num_spots) {
      ...
      //            printf("num batches %d batch %d spot start
      ...
      for (spot_ind = spot_start; spot_ind < this->num_spots; spot_ind++) {
        ...
        if (num_histories - current_history <
            histories_per_batch - current_vertex) {
          loop_end = num_histories - current_history + current_vertex;
        } else {
          loop_end = histories_per_batch;
        }
        /// The multithreading gives small performance gain if we need run it
        /// for each spot
        ...
        if (current_vertex == histories_per_batch &&
    ...
    printf("spot ind %lu num_spots %d cum vertices %lu total histories %lu\n",
           spot_ind, this->num_spots, cum_vertices, h1);
    printf("Number of particles tracked %d\n", tracked_particles[0]);

  } // run_by_spot

  virtual mqi::node_t<R> *
  create_rangeshifter(mqi::rangeshifter *geometry,
    ...
    printf("Printing rangeshifter specification.. : p_coord angle of "
           "rangeshifter --> \n(%f %f %f %f)\n",
           p_coord.angles[0], p_coord.angles[1], p_coord.angles[2],
           p_coord.angles[3]);

    printf("Printing rangeshifter specification.. : p_coord position of "
           "rangeshifter --> \n(%f %f %f)\n",
           p_coord.translation.x, p_coord.translation.y, p_coord.translation.z);

    mqi::coordinate_transform<R> p_final(p_coord.angles, p_coord.translation);

    std::cout << "Printing rangeshifter specification.. : Position -->"
              << std::endl;
    geometry->pos.dump();
    std::cout << "Printing rangeshifter specification.. : Volume -->"
              << std::endl;
    ...
    rangeshifter->geo = new grid3d<mqi::density_t, R>(
        ...
        geometry->pos.x + geometry->volume.x / 2, 2,
        geometry->pos.y - geometry->volume.y / 2,
        geometry->pos.y + geometry->volume.y / 2, 2,
        geometry->pos.z - geometry->volume.z / 2,
        geometry->pos.z + geometry->volume.z / 2, 2, p_final.rotation);

    std::cout << "Filling rangeshifter grid with specific density.."
              << std::endl;
    ...
    // std::cout << "Printing rangeshifter specification.. : Rotation for
    // coordinate transform -->" << std::endl; p_final.rotation.dump();
    std::cout << "Printing rangeshifter specification.. : Rotation for "
                 "rangeshifter geometry -->"
              << std::endl;
  ...
  CUDA_HOST_DEVICE
  bool sol1_1(mqi::vec2<float> pos, mqi::vec3<float> *&contour_points,
              int num_points) {
    ...
    for (i = 0, j = num_points - 1; i < num_points; j = i++) {
      ...
      if ((((pos0.y <= pos.y) && (pos.y < pos1.y)) ||
          ...
          (pos.x <
           (pos1.x - pos0.x) * (pos.y - pos0.y) / (pos1.y - pos0.y) + pos0.x)) {
        c = !c;
      }
    }
    return c;
  ...
  void fill_contour(uint8_t *volume_contour, mqi::vec3<float> *&contour_points,
                    int num_contour_points, mqi::vec3<ijk_t> dim,
                    const R *x_pix, const R *y_pix, const R *z_pix, float dx,
                    float dy) {
    int x_ind = 0, y_ind = 0, z_ind = -1;
    for (int i = 0; i < dim.z - 1; i++) {
      if (contour_points[0].z > z_pix[i] &&
          contour_points[0].z < z_pix[i + 1]) {
        z_ind = i;
        break;
      }
    }
  ...
  double *reshape_data(int c_ind, int s_ind, mqi::vec3<ijk_t> dim) {
    ...
    for (int ind = 0;
         ind < this->world->children[c_ind]->scorers[s_ind]->max_capacity_;
         ind++) {
      if (this->world->children[c_ind]->scorers[s_ind]->data_[ind].key1 !=
              mqi::empty_pair &&
          this->world->children[c_ind]->scorers[s_ind]->data_[ind].key2 !=
              mqi::empty_pair) {
        reshaped_data
            [this->world->children[c_ind]->scorers[s_ind]->data_[ind].key1] +=
  ...
  void save_reshaped_files() {
    ...
    for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
      for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers;
           s_ind++) {
        ...
        if (strcasecmp(this->output_format.c_str(), "mhd") == 0) {
          ...
          mqi::io::save_to_dcm<R>(this->world->children[c_ind], reshaped_data,
                                 this->particles_per_history,
                                 this->output_path, filename, vol_size,
                                 this->dcm_, this->twoCentimeterMode);
        }
        else {
    ...
    for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
      for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers;
        ...
        mqi::io::save_to_npz<R>(this->world->children[c_ind]->scorers[s_ind],
                                ...
                                filename, dim, this->num_spots);
      }
    }
    // auto                                      stop =
    // std::chrono::high_resolution_clock::now(); std::chrono::duration<double,
    // std::milli> duration = stop - start; printf("Reshape and save to npz done
    // %f ms\n", duration.count());
  }
