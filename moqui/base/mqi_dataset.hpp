#ifndef MQI_DATASET_HPP
#define MQI_DATASET_HPP
...
static const std::map<const modality_type,
                      const std::map<const std::string, const gdcm::Tag>>
    seqtags_per_modality = {
         ...
         }},
        {
            ...
            {{"beam", gdcm::Tag(0x300a, 0x03a2)},
             ...
             {"rsss",
              gdcm::Tag(0x300a, 0x0360)}, ///< range shifter setting sequence
             {"blk", gdcm::Tag(0x300a, 0x03a6)},
             {"comp", gdcm::Tag(0x300a, 0x0e2a)},
             {"ctrl",
              gdcm::Tag(0x300a, 0x03a8)}} ///< ion control point sequence
        },
        {IONRECORD,
         {
             ...
             {"machine",
              gdcm::Tag(0x300a, 0x0206)} ///< treatment machine record
         }}};
...
/// \note
/// Known-issues:
///  linker caused error of dupulication because this initialization is done in
///  header...
///   const gdcm::Dicts& dcm_object::dicts_ =
///   gdcm::Global::GetInstance().GetDicts();

class dataset {
protected:
  /// Tag, Name, and dataset container (recursive)
  std::vector<std::tuple<const gdcm::Tag, const std::string,
...
public:
  ...
  void dump() const {
    ...
    for (auto el = gdcm_ds_.Begin(); el != gdcm_ds_.End(); ++el) {
      ...
      std::cout << entry.GetKeyword() << ": " << el->GetTag()
                << ", VR: " << el->GetVR() << ", length: " << el->GetVL()
                << std::endl;
    }
  ...
  /// http://dicom.nema.org/dicom/2013/output/chtml/part05/sect_6.2.html \param
  /// vm DICOM Value Multiplicity:
  /// http://dicom.nema.org/dicom/2013/output/chtml/part05/sect_6.4.html \param
  /// vl Value length \param res reference to a "float" container for dicom
  /// values. \return void
  void get_values(const gdcm::ByteValue *bv, const gdcm::VR vr,
                  const gdcm::VM vm, const gdcm::VL vl,
  ...
  /// vm DICOM Value Multiplicity:
  /// http://dicom.nema.org/dicom/2013/output/chtml/part05/sect_6.4.html \param
  /// vl Value length \param res reference to a "int" container for dicom
  /// values. \return void
  void get_values(const gdcm::ByteValue *bv, const gdcm::VR vr,
                  const gdcm::VM vm, const gdcm::VL vl, std::vector<int> &res,
  ...
  /// vm DICOM Value Multiplicity:
  /// http://dicom.nema.org/dicom/2013/output/chtml/part05/sect_6.4.html \param
  /// vl Value length \param res reference to a "string" container for dicom
  /// values. \return void
  void get_values(const gdcm::ByteValue *bv, const gdcm::VR vr,
                  const gdcm::VM vm, const gdcm::VL vl,
  ...
  void get_values(const char *keyword, std::vector<T> &result) const {
    ...
    this->get_values(el.GetByteValue(), entry.GetVR(), entry.GetVM(),
                     el.GetVL(), result);
  }
