...
public:
  ...
  CUDA_HOST_DEVICE
  T cross(const vec2<T> &v) const { return x * v.y - y * v.x; }
  ...
  template <typename R> vec2<T> operator*(const R &r) const {
  ...
  CUDA_HOST_DEVICE
  void dump() const { printf("(x,y): (%f, %f)\n", x, y); }
};
...
public:
  ...
  CUDA_HOST_DEVICE
  vec3<T> operator*(const T &r) { return vec3<T>(x * r, y * r, z * r); }
...
public:
  ...
  CUDA_HOST
  vec4(const std::array<T, 4> &ref)
      : x(ref[0]), y(ref[1]), z(ref[2]), s(ref[3]) {
    ;
  }
  // #endif
  ...
  CUDA_HOST_DEVICE
  void dump() const { printf("(x,y,z,s): (%f, %f, %f)\n", x, y, z, s); }
};
