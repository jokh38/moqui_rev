#ifndef MQI_BEAM_MODULE_ION_HPP
#define MQI_BEAM_MODULE_ION_HPP

/// \file
///
/// Interprets DICOM-RT Ion beam module
/// \see
/// http://dicom.nema.org/medical/dicom/current/output/chtml/part03/sect_C.8.8.25.html
/// for RTI \see
/// http://dicom.nema.org/medical/dicom/current/output/chtml/part03/sect_C.8.8.26.html
/// for RTIBTR
...
public:
  ...
  beam_module_ion(const mqi::dataset *d, mqi::modality_type m)
      : beam_module(d, m) {
    ...
    /// As each layer consists of pair of ion-controls and even-layers have all
    /// zero weights. So we drop even layer.
    ...
    for (auto b : ictrl) {
      ...
      for (int j = 0; j < nb_pts[0]; ++j) {
        tune_id_ = tune_id[0];
        sequence_.push_back({energy[0], xy[j * 2], xy[j * 2 + 1], fwhm_xy[0],
                             fwhm_xy[1], weight[j]});
      } // per spot
  ...
  const std::vector<int> *get_nb_spots_per_layer(void) const {
