#ifndef MQI_TREATMENT_MACHINE_PBS_HPP
#define MQI_TREATMENT_MACHINE_PBS_HPP
...
#include "mqi_treatment_machine_ion.hpp"
...
namespace mqi {
template <typename R> class pbs_material_t : public patient_material_t<R> {
public:
  CUDA_HOST_DEVICE
...
template <typename T> class pbs : public treatment_machine_ion<T> {
protected:
  const T cm2mm;
  const T mm2cm;
...
public:
  ...
  virtual size_t characterize_history(const mqi::beam_module_ion::spot &s,
    ...
    T mid_ratio =
        intpl(s.e, beam_down->first, beam_up->first, down.ratio, up.ratio);
  ...
  /// this uses "phsp6d_fanbeam" distribution to sample position between x0 to
  /// x1.
  ...
  characterize_beamlet(const mqi::beam_module_ion::spot &s0,
    ...
    //        std::cout<<"Energy: "<< s.e <<", " << down.E << ", " << up.E << ",
    //        "<<mid_e<< "\n";
    ...
    mqi::vec3<T> pos0(0, 0,
                      mqi::treatment_machine_ion<T>::source_to_isocenter_mm_);
    ...
    std::array<T, 6> spot_pos_range = {pos0.x, pos1.x, pos0.y,
                                       pos1.y, pos0.z, pos1.z};
    std::array<T, 6> spot_sigma = {mid_x, mid_y, 0.0, mid_xp, mid_yp, 0.0};
    std::array<T, 2> corr = {0.0, 0.0};

    // spot direction mean is calculated from the sampled position between x0
    // and x1 so passing spot-range to a specialized distribution for handling
    // this case is prefered option.
  ...
  characterize_beamlet(const mqi::beam_module_ion::spot &s) {
    ...
    std::array<T, 6> beamlet_mean = {beam.x, beam.y, beam.z,
                                     dir.x,  dir.y,  dir.z};
  ...
  mqi::rangeshifter *characterize_rangeshifter(const mqi::dataset *ds,
    ...
    lxyz.y =
        geometry_spec_.rangeshifter[1]; // if this is 0, rangeshifter is circle
  ...
  /// \param s_curr current spot to be delivered. The irradiation time depends
  /// on meterset of this spot \param s_next next spot to be delivered. To
  /// calculate time to move from this to next spot. dT_move
  ...
  characterize_beamlet_time(const mqi::beam_module_ion::spot &s_curr,
    ...
    // dT_off: time to beam off including layer switing, settling spot,
    // latencies s_curr.meterset is number of particles
    ...
    if (&s_next != nullptr) {
      if (s_next.e != s_curr.e) {
        /// Layer changae.
        /// Note: The spot-settling time is not required or included in
  ...
  mqi::aperture *characterize_aperture(const mqi::dataset *ds,
    ...
    mqi::vec3<float> lxyz(geometry_spec_.aperture[0],
                          geometry_spec_.aperture[1], ftmp[0]); // 360 or 0?

    blk_ds[0]->get_values("IsocenterToBlockTrayDistance", ftmp);
    mqi::vec3<float> pxyz(0.0, 0.0, ftmp[0] + lxyz.z * 0.5);
    std::cout << "BlockThickness and Position (center) : " << lxyz.z << ", "
              << pxyz.z << " mm" << std::endl;
