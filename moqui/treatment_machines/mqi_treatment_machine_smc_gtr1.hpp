...
// namespace smc{
template <typename R> class gtr1_material_t : public patient_material_t<R> {
public:
...
/**
 * The <code>gtr1</code> class represents beam model for Sumitomo IMPT machine
 * in SMC
 */
template <typename T> class gtr1 : public treatment_machine_ion<T> {
protected:
public:
  ...
  gtr1() {
    ...
    std::vector<double> beamEnergyForParticleCountCalib = {
        70,  76,  84,  92,  100, 106, 112, 118, 126, 134,
        142, 146, 150, 154, 158, 162, 170, 174, 178, 182,
        186, 190, 198, 202, 206, 214, 218, 222, 226, 230};
    std::vector<double> correctionParticleCountCalib = {
        1,           1.065789513, 1.157943145, 1.2411556,   1.322953524,
        1.395086046, 1.458739464, 1.513003859, 1.588552029, 1.66206693,
        1.740579354, 1.780360004, 1.81434049,  1.848968773, 1.878347883,
        1.91131038,  1.971904065, 2.003357087, 2.02810976,  2.058108192,
        2.082444044, 2.108758102, 2.168636068, 2.20480403,  2.229879571,
        2.308058176, 2.342322973, 2.381755055, 2.447585594, 2.511318715};
    particleCountCalibInterp.set_points(beamEnergyForParticleCountCalib,
                                        correctionParticleCountCalib,
                                        tk::spline::cspline);
    ...
    std::vector<double> correctionBeamEnergySpread = {
        0.502793, 0.577766, 0.648957, 0.714868, 0.774127, 0.825533,
        0.868103, 0.901097, 0.92404,  0.936734, 0.939254, 0.931934,
        0.915346, 0.89027,  0.857653, 0.818572, 0.774184};
    beamEnergySpreadInterp.set_points(beamEnergyForBeamEnergySpread,
                                      correctionBeamEnergySpread,
                                      tk::spline::cspline);
    ...
    beamSpotSizeInterp.set_points(beamEnergyForBeamSpotSize,
                                  correctionBeamSpotSize, tk::spline::cspline);
    ...
    std::vector<double> correctionBeamAngularSpread = {
        0.004675, 0.00424,  0.00379,  0.003605, 0.003315, 0.003045,
        0.00294,  0.0028,   0.00253,  0.002435, 0.002285, 0.002205,
        0.002295, 0.00224,  0.002075, 0.002155, 0.002075, 0.001865,
        0.00196,  0.001855, 0.001885, 0.00195,  0.00205,  0.001945,
        0.002,    0.00184,  0.0018,   0.001795, 0.00171,  0.001695};
    beamAngularSpreadInterp.set_points(beamEnergyForBeamAngularSpread,
                                       correctionBeamAngularSpread,
                                       tk::spline::cspline);
    ...
    std::vector<double> correctionBeamDivergence = {
        0.0871,  0.0736,  0.0606, 0.0515,  0.0438,  0.0389,  0.0347,  0.0311,
        0.027,   0.0236,  0.0206, 0.0193,  0.018,   0.0164,  0.0154,  0.0144,
        0.0124,  0.0116,  0.0109, 0.0104,  0.00949, 0.00892, 0.00799, 0.00766,
        0.00739, 0.00671, 0.0065, 0.00638, 0.0066,  0.00697};
    beamDivergenceInterp.set_points(beamEnergyForBeamDivergence,
                                    correctionBeamDivergence,
                                    tk::spline::cspline);
  ...
  /// User method to characterize MODULATED beamlet based on spot information
  /// from DICOM.
  ...
  /// User method to characterize MODULATED beamlet based on spot information
  /// ans source to isocenter distnace from DICOM.
  virtual mqi::beamlet<T>
  characterize_beamlet(const mqi::beam_module_ion::spot &s,
  ...
  /// User method to characterize UNIFORM/MODULATED_SPEC beamlet based on spot
  ...
  // Particle count in MC = MU Count * (Dose / MU Count) * (Particle / dose) *
  // (Dose monitor range--> implemented in pre-program)
  size_t characterize_history(const mqi::beam_module_ion::logspot &s) {
    /* SMC Log file version */
    // std::cout << "Beam energy : " << s.e << std::endl;
    // std::cout << "Proton / dose interp : " << protonPerDoseInterp(s.e) <<
    // std::endl; std::cout << "Dose Per MU count interp : " <<
    // dosePerMUCountInterp(s.e) << std::endl;
    int particleFromMUCount =
        s.muCount *
        particleCountCalibInterp(
  ...
  // Using mqi::beam_module_ion::logspot, with an added feature in
  // mqi_beam_module_ion Added by Chanil Jeon
  mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::logspot &s,
    ...
    std::array<T, 6> beamlet_sigm = {spotSize,      spotSize,      0,
                                     angularSpread, angularSpread, 0};
    std::array<T, 2> beamlet_divergence = {divergence, divergence};
    auto beamlet = new mqi::phsp_6d_ray<T>(
        beamlet_mean, beamlet_sigm, beamlet_divergence, newBeamStartingPos);

    return mqi::beamlet<T>(energy, beamlet);
  }

  mqi::rangeshifter *characterize_rangeshifter(const mqi::dataset *ds,
                                               mqi::modality_type m) {
  ...
  mqi::aperture *characterize_aperture(const mqi::dataset *ds,
                                       mqi::modality_type m) {
  ...
  create_beamsource(const mqi::logfiles_t &logfileData,
    ...
    for (int i = 0; i < logfileData.beamInfo.size(); i++) {
      for (int j = 0; j < logfileData.beamInfo[i].size(); j++) {
        ...
        logSpotInfo.e =
            logfileData.beamEnergyInfo[i][j]; // Log file spot energy (single
                                              // energy layer)
        ...
        for (int k = 0; k < logfileData.beamInfo[i][j].muCount.size(); k++) {
          ...
          beamsource.append_beamlet_log(
              this->characterize_beamlet(
                  logSpotInfo, treatment_machine<T>::source_to_isocenter_mm_,
                  rsuse),
