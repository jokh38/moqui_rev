...
template <typename R>
CUDA_DEVICE void insert_hashtable(mqi::key_value *hashtable, mqi::key_t key1,
                                  mqi::key_t key2, double value,
...
template <typename R>
CUDA_GLOBAL void transport_particles_patient(
    mqi::thrd_t *threads, mqi::node_t<R> *world, mqi::vertex_t<R> *vertices,
    const uint32_t n_vtx, uint32_t *tracked_particles,
    ...
    while (!stack.is_empty()) {
      ...
      for (c_ind = 0; c_ind < world->n_children; c_ind++) {
        ...
        while (c_geo.is_valid(track.its.cell) && !track.is_stopped()) {
          ...
          track.its =
              c_geo.intersect(track.vtx0.pos, track.vtx0.dir, track.its.cell);
          rho_mass = c_geo[cnb];
          ...
          if (!track.primary && track.dE > 0) {
            ...
            fippel.stepping(track, stack, thread_rng, rho_mass, water,
                            track.its.dist, score_local_deposit);
          }
#else
          fippel.stepping(track, stack, thread_rng, rho_mass, water,
                          track.its.dist, score_local_deposit);
...
CUDA_GLOBAL void transport_particles_patient_seed(
    mqi::thrd_t *threads, mqi::node_t<R> *world, mqi::vertex_t<R> *vertices,
    const uint32_t n_vtx, uint32_t *tracked_particles, int32_t *transport_seed,
    //                                 uint16_t*         mat_ids,
    uint32_t *scorer_offset_vector = nullptr, bool score_local_deposit = true,
    uint32_t total_threads = 1, // # of CPU threads
    uint32_t thread_id = 0)     // CPU thread-id
